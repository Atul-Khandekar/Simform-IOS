...
class LoginViewModel: BaseViewModel {
    ...
    func callApi(email: String , password: String) {
        let userDetails = LoginModel(email: email, password: password)
        callLoginApi(requestPayload: userDetails.convertToData(), requestType: .loginUser) { [weak self] (result: Result<LoginResponseModel, Error>) -> ()  in
            switch result {
